...
#### 1. 모든 함수가 `prototype`을 가지므로 불필요한 리소스 낭비
...
#### <br>**그리하여 ES6는 함수의 사용 목적에 따라 세 가지 종류로 명확히 구분하였다.**
...
### 2-1. "화살표 함수가 일반함수와 구별되는 가장 큰 특징은 `this`다."
...
### 2-4) 화살표 함수에서는 argument 객체를 사용할 수 없다.

#### 1. 즉시실행 함수의 arguments
...
#### 3. 화살표 함수에서 arguments 대신 Rest 파라미터 사용
...
### 4) 매개변수 기본값과 함수 `length` 및 `arguments`
